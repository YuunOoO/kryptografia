class CryptoLogic {
  ...
  for (int i = 0; i < bytes.length; i += 3) { // Iteruj przez listę bajtów w grupach po 3
    final int value = ((bytes[i] & 0xFF) << 16) | // Tworzenie 24-bitowej wartości z trzech bajtów
        ((i + 1 < bytes.length ? bytes[i + 1] & 0xFF : 0) << 8) |      // sprawdza, czy istnieje co najmniej jeden kolejny bajt po obecnym w tablicy bajtów bytes.
        ((i + 2 < bytes.length ? bytes[i + 2] & 0xFF : 0));

    for (int j = 0; j < 4; j++) { // Iteruj po 4 znaki Base64
      if (i + j <= bytes.length) {
        final int index = (value >> (18 - j * 6)) & 0x3F; // Wybierz indeks znaku w alfabecie Base64
        result.write('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'[index]); // Dodaj znak Base64 do wyniku
      } else {
  ...
  return result.toString().substring(0, result.length - padding); // Zwróć wynik jako ciąg Base64 z usuniętym paddingiem
}
